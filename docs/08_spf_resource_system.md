...
| 文档版本 | 日期 | 作者 | 说明 |
|---------|------|------|------|
| 1.0 | 2026-02-14 | AI Assistant | 基于源代码文档 |
| 2.0 | 2026-02-14 | AI Assistant | 基于实际文件分析更新 F_SPF_HEADER 大小 |
| 3.0 | 2026-02-14 | AI Assistant | 添加实际文件分析示例和版本号格式说明 |
...
| -140 | F_SPF_HEADER | 136 字节 | spfHeader | SPF 文件头 |
| -(140 + 索引) | FINFO[] | 可变 | 文件索引表 | N 个文件信息结构 |
| 0 ~ (文件大小 - 索引 - 140) | byte[] | 可变 | 文件数据 | 实际的打包文件数据 |

#### 文件定位计算

```
文件数据区大小 = 文件总大小 - 4 - 136 - 索引表大小
索引表开始位置 = 文件总大小 - 4 - 136 - 索引表大小
SPF头位置 = 文件总大小 - 4 - 136 = -140
版本号位置 = 文件总大小 - 4 = -4
FILE_ID位置 = 文件总大小 - 136 = -136 (从-140开始的第4-8字节)
```
...
**版本号格式**：实际分析显示版本号采用日期格式，例如 `2022091501`，表示 2022年9月15日 的第 01 版。
...
SPF 文件通过 FILE_ID（RESID 的高 8 位）进行标识，游戏启动时按照固定的 FILE_ID 顺序加载 SPF 文件。

| FILE_ID | SPF 文件名 | 资源类型 | 状态 | 说明 |
|---------|-----------|---------|------|------|
| 0 | TESTPACK.SPF | 测试 | ✅ 启用 | 测试资源包 |
| 1 | -AJJIYA.SPF | 角色 | ❌ 禁用 | 文件名带 - 前缀表示不加载 |
| 2 | HOSHIM.SPF | 角色 | ✅ 启用 | 角色资源 |
| 3 | ROWID.SPF | 角色 | ✅ 启用 | 角色资源 |
| 4 | -JINSSAGA.SPF | 角色 | ❌ 禁用 | 文件名带 - 前缀表示不加载 |
| 5 | MAKO1298.SPF | 角色 | ✅ 启用 | 角色资源 |
| 6 | METALGENI.SPF | 角色 | ✅ 启用 | 角色资源 |
| 7 | DALBONG.SPF | 角色 | ✅ 启用 | 角色资源 |
| 8 | RYUMS.SPF | 角色 | ✅ 启用 | 角色资源 |
| 9 | BANX.SPF | 角色 | ✅ 启用 | 角色资源 |
| 10 | BARY.SPF | 角色 | ✅ 启用 | 角色资源 |
| 11 | -ZENNE.SPF | 角色 | ❌ 禁用 | 文件名带 - 前缀表示不加载 |
| 12 | CLAIRE.SPF | 角色 | ✅ 启用 | 角色资源 |
| 13 | CVOICE.SPF | 音频 | ✅ 启用 | 语音资源 |
| 14 | GUSTAV.SPF | 角色 | ✅ 启用 | 角色资源 |
| 15 | CRI.SPF | 音频 | ✅ 启用 | 音频资源（CRI Middleware） |
| 16 | DURAGON.SPF | 角色 | ✅ 启用 | 角色资源 |
| 17 | -TOMA.SPF | 角色 | ❌ 禁用 | 文件名带 - 前缀表示不加载 |
| 18 | CLETS.SPF | 角色 | ✅ 启用 | 角色资源 |
| 19 | BORORU.SPF | 角色 | ✅ 启用 | 角色资源 |
| 20 | JOOX3.SPF | 角色 | ✅ 启用 | 角色资源 |
| 21 | BONGSIK.SPF | 角色 | ✅ 启用 | 角色资源 |
| 22 | RM.SPF | 地图/关卡 | ✅ 启用 | 地图资源 |
| 23 | JJALRAJO.SPF | 特效 | ✅ 启用 | 特效资源 |
| 24 | JJANG.SPF | 特效 | ✅ 启用 | 特效资源 |
| 25 | -FREE.SPF | 未知 | ❌ 禁用 | 文件名带 - 前缀表示不加载 |
| 26 | -BAS.SPF | 未知 | ❌ 禁用 | 文件名带 - 前缀表示不加载 |
| 27 | BUGLE.SPF | 角色 | ✅ 启用 | 角色资源 |
| 28 | CHIRS.SPF | 角色 | ✅ 启用 | 角色资源 |
| 29 | LILY.SPF | 角色 | ✅ 启用 | 角色资源 |
| 30 | FUYU.SPF | 角色 | ✅ 启用 | 角色资源 |
| 31 | JMULRO.SPF | 角色 | ✅ 启用 | 角色资源 |
| 32 | CC.SPF | 角色 | ✅ 启用 | 角色资源 |
| 33 | CW.SPF | 角色 | ✅ 启用 | 角色资源 |
| 34 | JX.SPF | 角色 | ✅ 启用 | 角色资源 |
| 35 | PCZ.SPF | 角色 | ✅ 启用 | 角色资源 |
| 36 | BJF.SPF | 角色 | ✅ 启用 | 角色资源 |

### RESID 计算

FILE_ID 用于 RESID（资源 ID）的编码：

```
RESID = (FILE_ID << 24) | INSTANCE_ID
```

例如：
- `HOSHIM.SPF` 中的资源：`RESID = 0x02XXXXXX`（FILE_ID = 2）
- `CVOICE.SPF` 中的资源：`RESID = 0x0DXXXXXX`（FILE_ID = 13）
- `RM.SPF` 中的资源：`RESID = 0x16XXXXXX`（FILE_ID = 22）
...
格式: [目录名/文件名.扩展名]
示例:
- "DATA/ANITABLE/010_01_01_STAND.DDT"
- "DATA/ANITABLE/010_01_01_STAND.SDT"
...
**文件数量验证**：
```
索引表大小 / FINFO大小 = 2,946,860 / 140 = 21,049 ✓
```

**典型文件条目示例**：

| 序号 | 文件名 | 偏移量 | 大小 | RESID | FILE_ID | INSTANCE_ID |
|-----|--------|-------|------|-------|---------|-------------|
| 1 | DATA/ANITABLE/010_01_01_STAND.DDT | 0 | 48 | 0x01000001 | 1 | 1 |
| 2 | DATA/ANITABLE/010_01_01_STAND.SDT | 48 | 3,424 | 0x01000002 | 1 | 2 |
| 3 | DATA/ANITABLE/010_01_02_MOVE.DDT | 3,472 | 48 | 0x01000003 | 1 | 3 |
| ... | ... | ... | ... | ... | ... | ... |

**RESID 编码验证**：
```
RESID = (FILE_ID << 24) | INSTANCE_ID
0x01000001 = (1 << 24) | 1 = 0x01000000 | 0x00000001 ✓
```
